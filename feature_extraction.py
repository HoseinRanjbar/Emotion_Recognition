...
train_csv = pd.read_csv(os.path.join('./tools/data/original_csv','train.csv'))
...
for d in range(len(sizes)):
    ...
    score_file = np.zeros((size,args.num_classes))
    ...
    for step, (x, x_lengths, y, hand_regions, hand_lengths) in enumerate(dataloader):
        ...
        score_file[step] = class_logits.cpu().detach().numpy()
        # print(pred)
    ...
    np.save('{}/{}_score.npy'.format(save_path, dataset[d]), score_file)
